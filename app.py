...
from firebase_admin import credentials, firestore, initialize_app, firestore, exceptions
...
# Enable CORS
# TODO: Improve security by specifying the allowed origins; CORS(app, resources={r"/add": {"origins": "https://tudominio.com"}})
...
class URLSchema(Schema):
    url = fields.Url(required=True, error_messages={"required": "La URL es obligatoria", "invalid": "La URL proporcionada no es válida"})
...
def crear_usuario_admin():
    ...
    success, result = crear_usuario(username, password)
    if success:
        return jsonify({'message': 'Usuario creado exitosamente.', 'user_id': result}), 201
    else:
        return jsonify({'message': result}), 400
...
def _validar_url(url):
    ...
    if len(url) > MAX_URL_LENGTH:
        abort(400, description=f'La URL excede la longitud máxima permitida de {MAX_URL_LENGTH} caracteres')

    parsed_url = urlparse(url)

    if not (parsed_url.scheme and parsed_url.netloc):
        abort(400, description='La URL debe contener un esquema (http o https) y un dominio válido')
...
def obtener_qr(uuid):
    ...
    try:
        ...
        return send_file(qr_image, mimetype='image/webp', as_attachment=False, download_name=f"{uuid}.webp")
    except Exception as e:
